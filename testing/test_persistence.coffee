...
getDBInstance = () ->
  n = i++
  dbs[n] = require path.join '..', 'js-coffee', 'persistence'
  opts =
    logger: log
  opts[ 'db-port' ] = 6379
  dbs[n] opts
...
exports.EventPoller =
  ...
  tearDown: ( cb ) =>
    n = i++
    dbs[n] = getDBInstance()
    dbs[n].eventPollers.deleteModule @event1id
    dbs[n].eventPollers.deleteModule @event2id
    setTimeout dbs[n].shutDown(), 500
    cb()
