...
exports.init = (function(_this) {
  return function(oConf) {
    ...
    if (!_this.db) {
      ...
      _this.db = redis.createClient(oConf.port, 'localhost', {
        connect_timeout: 2000
      });
      _this.db.on('error', function(err) {
        if (err.message.indexOf('ECONNREFUSED') > -1) {
          _this.connRefused = true;
          return log.warn('DB:REDIS§ | Wrong port?');
        } else {
...
exports.isConnected = (function(_this) {
  return function(cb) {
    ...
    if (!_this.db) {
      return cb(new Error('DB:REDIS | DB initialization did not occur or failed miserably!'));
    } else {
      if (_this.db.connected) {
        ...
        fCheckConnection = function() {
          ...
          if (_this.connRefused) {
            ...
            return cb(new Error('DB:REDIS | Connection refused! Wrong port?'));
          } else {
            if (_this.db.connected) {
              ...
              return cb(new Error('DB:REDIS | Connection to DB failed!'));
            }
          }
        };
        return setTimeout(fCheckConnection, 100);
  ...
  return function(action) {
    return function(err, reply) {
      if (err) {
        ...
        return log.info("DB:REDIS | " + action + ": " + reply);
      }
    };
  };
})(this);
...
IndexedModules = (function() {
  function IndexedModules(setname, log) {
    ...
    log.info("DB:REDIS | (IdxedMods) Instantiated indexed modules for '" + this.setname + "'");
  }

  IndexedModules.prototype.setDB = function(db1) {
    this.db = db1;
    return log.info("DB:REDIS | (IdxedMods) Registered new DB connection for '" + this.setname + "'");
  };
  ...
  IndexedModules.prototype.storeModule = function(userId, oModule) {
    log.info("DB:REDIS | (IdxedMods) " + this.setname + ".storeModule( " + userId + ", oModule )");
  ...
  IndexedModules.prototype.getModule = function(userId, mId, cb) {
    log.info("DB:REDIS | (IdxedMods) " + this.setname + ".getModule( " + userId + ", " + mId + " )");
  ...
  IndexedModules.prototype.getAllModules = function(userId, cb) {
    var gM;
    log.info("DB:REDIS | (IdxedMods) " + this.setname + ".getAllModules( " + userId + " )");
    gM = function(mId) {
  ...
  IndexedModules.prototype.storeUserParams = function(mId, userId, encData) {
    log.info("DB:REDIS | (IdxedMods) " + this.setname + ".storeUserParams( " + mId + ", " + userId + ", encData )");
  ...
  IndexedModules.prototype.storeUserArguments = function(userId, ruleId, mId, funcId, encData) {
    log.info("DB:REDIS | (IdxedMods) " + this.setname + ".storeUserArguments( " + userId + ", " + ruleId + ", " + mId + ", " + funcId + ", encData )");
  ...
  IndexedModules.prototype.getUserArgumentsFunctions = function(userId, ruleId, mId, cb) {
    log.info("DB:REDIS | (IdxedMods) " + this.setname + ".getUserArgumentsFunctions( " + userId + ", " + ruleId + ", " + mId + " )");
  ...
  IndexedModules.prototype.getAllModuleUserArguments = function(userId, ruleId, mId, cb) {
    log.info("DB:REDIS | (IdxedMods) " + this.setname + ".getAllModuleUserArguments( " + userId + ", " + ruleId + ", " + mId + " )");
  ...
  IndexedModules.prototype.getUserArguments = function(userId, ruleId, mId, funcId, cb) {
    log.info("DB:REDIS | (IdxedMods) " + this.setname + ".getUserArguments( " + userId + ", " + ruleId + ", " + mId + ", " + funcId + " )");
  ...
  IndexedModules.prototype.deleteUserArguments = function(userId, ruleId, mId) {
    log.info("DB:REDIS | (IdxedMods) " + this.setname + ".deleteUserArguments( " + userId + ", " + ruleId + ", " + mId + " )");
    return this.db.smembers(this.setname + ":" + userId + ":" + ruleId + ":" + mId + ":functions", (function(_this) {
      return function(err, obj) {
...
exports.getAllUserWebhooks = (function(_this) {
  return function(username, cb) {
    return getSetRecords("user:" + username + ":webhooks", exports.getFullWebhook, cb);
  };
})(this);
